...
export const ExchangeRateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    // Initialize with historical rates from localStorage
    const [rateHistory, setRateHistory] = useState<ExchangeRateRecord[]>(() => {
        const saved = localStorage.getItem('exchange_rate_history');
        if (saved) {
            return JSON.parse(saved);
        }

        // Initialize with today's default rates
    ...
    useEffect(() => {
        localStorage.setItem('exchange_rate_history', JSON.stringify(rateHistory));
    }, [rateHistory]);
