...
const roomsDetailsSchema = z.object({
    ...
    pricePerPersonDzd: z.number().positive({ message: 'Price per person must be positive' }),
...
const visaDetailsSchema = z.object({
    quantity: z.number().int().positive({ message: 'Quantity must be a positive integer' }),
    pricePerVisa: z.number().positive({ message: 'Price per visa must be positive' }),
...
const transportationDetailsSchema = z.object({
    ...
    quantity: z.number().int().positive({ message: 'Quantity must be a positive integer' }),
    pricePerUnit: z.number().positive({ message: 'Price per unit must be positive' }),
...
const flightDetailsSchema = z.object({
    ...
    ticketQuantity: z.number().int().positive({ message: 'Ticket quantity must be a positive integer' }),
    pricePerTicket: z.number().positive({ message: 'Price per ticket must be positive' }),
    departure: z.object({
...
const foodDetailsSchema = z.object({
    ...
    quantity: z.number().int().positive({ message: 'Quantity must be a positive integer' }),
    pricePerMeal: z.number().positive({ message: 'Price per meal must be positive' }),
    dateFrom: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
    dateTo: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
