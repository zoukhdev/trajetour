...
class OfflineQueueService {
    ...
    private async loadQueue() {
        try {
            const stored = await clientStorage.getItem(QUEUE_KEY);
            if (stored) {
    ...
    private async saveQueue() {
        try {
            await clientStorage.setItem(QUEUE_KEY, JSON.stringify(this.queue));
    ...
    public async addRequest(req: Omit<OfflineRequest, 'id' | 'timestamp' | 'retryCount'>) {
